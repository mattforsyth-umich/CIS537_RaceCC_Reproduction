...
def main():
    ...
    # Configure bottleneck queue on port 5 (facing h5)
    # Must run after net.start() and sleep(1) so BMv2 is ready
    thrift_port = args.thrift_port
    setup_cmds = "\n".join([
        "set_queue_rate 250 5",
        "set_queue_depth 64 5",
        "register_reset fair_rate",
        "register_reset tx_bytes",
        "register_reset tx_bytes_last",
        "register_reset last_update",
        "register_reset inc_flag",
    ])
    os.system(f'echo "{setup_cmds}" | simple_switch_CLI --thrift-port {thrift_port}')
    print(f"[setup] Bottleneck configured: queue_rate=280pps, queue_depth=64 on port 5")
