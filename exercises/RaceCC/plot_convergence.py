...
# A flow counts as converged when it stays within 20% of fair share
# for at least 5 samples in a row
TOLERANCE        = 0.2
STABLE_WINDOW    = 5
BOTTLENECK_KBPS  = 575
...
def measure_convergence(flows, event_time, active_ips, target_rate):
    # parse each packets rate after an event and find when it stabilized
    # All flows have to converge before the event is considered converged
    per_flow = []
    ...
    # Use the slowest flow since convergence requires all flows to be stable
...
def get_convergence_times(flows):
    ...
    # Every flow after the first one is a join event
    for i, (ip, t) in enumerate(sorted(starts.items(), key=lambda x: x[1])):
        if i == 0:
            continue
        events.append((t, 'join', ip, i, i + 1))

    # Any flow that stopped before the experiment ended is a leave event
...
def plot_cdf(all_events, out_path='convergence_cdf.png'):
    ...
    ax.set_ylabel('CDF')
    ax.set_title('RaceCC -- CDF of Convergence Time')
    ax.set_ylim(0, 1.05)
    ax.set_xlim(left=0)
    ax.legend()
...
def plot_rates(flows, out_path='rate_fairshare.png'):
    ...
    # Recompute what fair share should be at each second based on active flows
    fair_x, fair_y = [], []
    for t in all_times:
    ...
    # Mark join and leave events so its easy to see what triggered each transition
    ...
    ax.set_title('RaceCC -- Rate vs Fair Share')
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def main():
    ...
    for path in paths:
        ...
        # Keep the first run for the rate plot
